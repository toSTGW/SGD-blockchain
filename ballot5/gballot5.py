...
web3 = Web3(Web3.HTTPProvider(const.GANACHE_URL, request_kwargs={'timeout': 200}))

account_address = web3.eth.accounts[account_number]
...
X_train = X[account_number*18000 : (account_number+1)*18000]
y_train = y[account_number*18000 : (account_number+1)*18000]
...
while True:
    ...
    transaction.update({'gas': 10000000})
    transaction.update({'gasPrice': web3.eth.gasPrice})
    ...
    try:
        ...
        print("user %d upload_param_txn_receipt:"%(account_number), txn_receipt)
    ...
    txn_hash = web3.eth.sendTransaction(transaction)
    try:
        print("user %d vote_txn_receipt:"%(account_number), txn_receipt)
        txn_receipt = web3.eth.waitForTransactionReceipt(txn_hash)
        if txn_receipt['status'] == 1:
    ...
    monitor_event('vote_statistics_complete')
