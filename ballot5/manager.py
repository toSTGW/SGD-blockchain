...
# web3 = Web3(Web3.WebsocketProvider(const.GANACHE_URL, websocket_kwargs={'timeout': 300}))
web3 = Web3(Web3.HTTPProvider(const.GANACHE_URL, request_kwargs={'timeout': 300}))
...
# ganache中的账户
account_address = "0x62262636f49268d3B234DA03Cf301D7bC1BAd9b7"
account_private_key = "a968aab0118891b6981c5b9b76474652157e2714bd8a55d6162d21b25b380e28"
...
transaction.update({'gas': const.GAS})
...
print("已经在合约中存入%.2f Ether作为奖励" % (train_value))
...
def monitor_event(state, poll_interval = 1):
    while True:
        tmp = -1
...
for i in range(valid_user_num):
    ...
    s_time = time.time()
    #等待所有用户上传参数
    monitor_event('param_upload_complete')
    ...
    monitor_event('vote_upload_complete')
    ...
    e_time = time.time()
    print("第%.2f轮训练用时%.2f秒" % (i, e_time - s_time))

print("所有用户退出训练")

end_time = time.time()

print("训练结束，共计用时%.2f秒" % (end_time - start_time))

print("测试训练结果")
df = pd.read_csv('E:/Data/creditcard/creditcard.csv')
